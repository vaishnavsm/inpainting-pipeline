...
def binarise_saliency_map(saliency_map,method='adaptive',threshold=0.5):
	...
	if method == 'fixed':
		...
		adaptive_threshold = 2.0 * threshold * saliency_map.mean()
		return (saliency_map > adaptive_threshold)

	elif method == 'cv2':
		(T, saliency) = cv2.threshold(saliency_map, 200, 255, cv2.THRESH_BINARY)
		return saliency

	else:
		print("Method must be one of fixed, adaptive")
		return None
...
if __name__ == '__main__':
	sal = get_saliency_mbd(sys.argv[1]).astype('uint8')
	binarised = 255*binarise_saliency_map(sal, method="adaptive", threshold=THRESHOLD).astype('uint8') #Thresholding
	#binarised = binarise_saliency_map(sal, method="cv2", threshold=0.8).astype('uint8')
	cv2.imwrite(sys.argv[2], binarised)
	cv2.imwrite((sys.argv[2].split('.')[0])+"-full-saliency.png", sal)
